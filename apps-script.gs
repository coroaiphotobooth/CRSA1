...
/**
 * BACKEND UNTUK CORO AI PHOTOBOOTH - V11.3 (Fix Manual Run Error)
 * Fitur: Session, Queue, Video Upload Handling, Incremental Sync, Atomic Locking
 * IMPORTANT: DEPLOY AS WEB APP (Access: Anyone)
 */

const SCRIPT_PROP = PropertiesService.getScriptProperties();

// --- ENTRY POINTS (Placed at top to avoid accidental manual runs of helpers) ---
  ...
  if (action === 'getSettings') {
    ...
    return createJsonResponse({
      ...
      settings: {
        ...
        videoPrompt: SCRIPT_PROP.getProperty('VIDEO_PROMPT') || 'Apply slow camera movement (push-in, push-out, pan, or parallax depth effect). Add subtle natural motion to the subject such as blinking, breathing, and micro expressions. Keep the face sharp, realistic, and undistorted.',
...
function doPost(e) {
  try {
      ...
      if (action === 'finalizeVideoUpload') {
         ...
         // [CRITICAL] ATOMIC PRE-CHECK (Idempotency)
         // Check if upload is already done/exists BEFORE doing heavy fetch
         ...
         // [PARALLEL PART] Fetch & Upload to Drive (Only if Pre-Check passed)
         let fileId = null;
         let folderId = null;
         ...
         try {
             ...
             // OPTIONAL: Mark as failed so retry can happen or stop
             executeWithLock(() => {
                 const gallerySheet = ensureGallerySheet(ss);
         ...
         return executeWithLock(() => {
             ...
             for (let i = 1; i < values.length; i++) {
                 if (values[i][idIndex] === data.photoId) {
                     ...
                     // [CRITICAL FIX] ATOMIC STATE CHECK
...
/**
 * Executes a callback within a lock to ensure atomic sheet/property writes.
 * Used only for the specific block of code that modifies shared resources.
 */
function executeWithLock(callback, timeoutMs = 30000) {
  // GUARD: Prevent manual execution from Editor where callback is undefined
  if (typeof callback !== 'function') {
    throw new Error("❌ SYSTEM ERROR: executeWithLock called without a function. \n\n⚠️ DO NOT RUN THIS FUNCTION MANUALLY.\nTo deploy, click 'Deploy' -> 'New Deployment'.\nTo test, use the 'doGet' function.");
  }
  ...
  try {
    ...
    if (!hasLock) {
      throw new Error("Server busy (Lock timeout). Please try again.");
    }
    return callback();
  } finally {
    lock.releaseLock();
