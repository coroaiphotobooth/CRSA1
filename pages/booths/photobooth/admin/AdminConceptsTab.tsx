...
const AdminConceptsTab = forwardRef<AdminConceptsTabRef, AdminConceptsTabProps>(({ concepts, onSaveConcepts, adminPin, settings }, ref) => {
  ...
  const handleDoneEditing = async (index: number) => {
    ...
    const isTemplateOrSmart = concept.id?.startsWith('smart_') || concept.concept_id?.startsWith('smart_');
    ...
    // A standard prompt generated by our AI will usually contain structural keywords
    const isStandardized = promptText.includes('AUTO-DETECT SUBJECT') && promptText.includes('OUTFIT');
    const isTooShort = promptText.length < 50;

    // We trigger silent optimization if:
    // 1. It's not standard AND (it's too short OR they uploaded images)
    if (!isStandardized && ((hasAnyRefImage && promptText.length > 0) || isTooShort)) {
      ...
      try {
        // Proceed with silent optimization
        console.log("Silently optimizing prompt...");
        const apiKey = process.env.GEMINI_API_KEY || process.env.API_KEY || import.meta.env.VITE_GEMINI_API_KEY;
        ...
        console.error("Silent optimization failed:", err);
        // Ignore errors, just proceed with closing
      } finally {
        setIsSilentlyOptimizing(null);
        setEditingConceptIndex(null);
      }
    } else {
      // Already standardized, or long enough without images. Just close.
      setEditingConceptIndex(null);
    }
  };
